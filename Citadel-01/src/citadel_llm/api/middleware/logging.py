...
class LoggingMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, config: dict):
        ...
        middleware_config = config.get('middleware', {}).get('logging', {})
        self.logging_enabled = middleware_config.get('enabled', True)  # Default to enabled
        self.include_body = middleware_config.get('include_body', False)
        self.log_level_str = middleware_config.get('level', 'info').upper()
