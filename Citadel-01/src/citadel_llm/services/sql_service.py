...
class SQLService:
    ...
    async def get_conversation_history(self, conversation_id: str) -> Dict[str, Any]:
        """Retrieve complete conversation history with messages"""
        try:
            async with self.get_connection() as conn:
                # Get conversation details
                conversation = await conn.fetchrow("""
                    SELECT id, user_id, model_name, title, start_time, end_time,
                           total_tokens, duration_ms, status, metadata, created_at
                ...
                messages = await conn.fetch("""
                    SELECT id, role, content, token_count, model_name, 
                           message_order, timestamp, metadata
                    FROM messages 
    ...
    async def get_model_statistics(self, model_name: str = None, days: int = 30) -> List[Dict[str, Any]]:
            ...
            async with self.get_connection() as conn:
                if model_name:
                    ...
                    stats = await conn.fetch("""
                        SELECT model_name, request_count, total_tokens, 
                               avg_response_time_ms, last_used, date_recorded
                    ...
                    stats = await conn.fetch("""
                        SELECT model_name, SUM(request_count) as total_requests,
    ...
    async def update_model_stats(self, model_name: str, tokens_used: int, 
        ...
        """Update model usage statistics for monitoring and analytics"""
        try:
            async with self.get_connection() as conn:
                # Use UPSERT to update existing record or create new one
                await conn.execute("""
                    INSERT INTO model_usage_stats 
                    (model_name, request_count, total_tokens, avg_response_time_ms, last_used, date_recorded)
                    VALUES ($1, 1, $2, $3, CURRENT_TIMESTAMP, CURRENT_DATE)
                    ...
                    DO UPDATE SET
                        ...
                        avg_response_time_ms = (
                            (model_usage_stats.avg_response_time_ms * model_usage_stats.request_count + EXCLUDED.avg_response_time_ms) /
                            (model_usage_stats.request_count + 1)
                        ),
                        last_used = CURRENT_TIMESTAMP
                """, model_name, tokens_used, response_time_ms)
                
                logger.debug(f"Model stats updated for {model_name}: {tokens_used} tokens, {response_time_ms}ms")
                
        except Exception as e:
            logger.error(f"Failed to update model statistics: {e}")
            raise HTTPException(status_code=500, detail="Failed to update model statistics")
